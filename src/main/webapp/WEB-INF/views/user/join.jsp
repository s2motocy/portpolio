...
$(document).ready(function(){
    ...
    function check(){
	    ...
	    $("button[type='submit']").click(function(e){
	    	if($("#user_id").val() == "" || $("#pwd").val() == "" || $("#user_name").val() == "" || $("#phone").val() == "" || $("#email").val() == "" ){                
                alert("공백을 입력하세요.");               
                return false;
            }else if ($("#pwd").val() != $("#pwd2").val()){
                alert("비밀번호가 서로 일치하지 않습니다.");
                return false;
            }else if(!checkString.test($("#user_id").val()) || !checkString.test($("#pwd").val()) || !checkName.test($("#user_name").val())) {
                alert("형식에 맞춰 작성해주세요.")                               
                return false;
            }else {
                alert("회원가입에 성공하였습니다. :)");
	    ...
	    <div>
	    	<form  action="join" method="post" class="form-horizontal" id="signupForm">
				...
				<label for="pwd">*비밀번호</label> 
		            	<input type="password" class="user-input" name="pwd" name="pwd" id="pwd"  placeholder="4~12자의 영문 소문자와 숫자로만 입력" maxlength="12" required/>
		        </div>
                ...
                <div class="update_cul">
		        	<label for="address">주소</label><br>
			            ...
			            <input type="text" class="user-input" id="post_code" name="post_code" value="${vo.post_code}"/>
